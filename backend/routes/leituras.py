...
@router.get("/")
async def get_leituras() -> JSONResponse:
...
async def add_leitura(dados:Dados):
    if dados.rua is None:
        dados.rua, dados.tipo_zona = await obter_endereco(dados.latitude, dados.longitude)
    
    timestamp = datetime.strptime(dados.timestamp, "%d-%m-%Y %H:%M:%S") if dados.timestamp != None else datetime.now() 
